...
SELECT split(entry, ":")[0],split(entry, ":")[1], split(entry, ":")[2],
split(entry, ":")[3],split(entry, ":")[4], split(entry, ":")[5]
FROM strange_string LATERAL VIEW explode(rowWithHeader(split(strange, "\\|"), 0)) entryTable AS entry;

SELECT split(t1.entry, ":")[0],split(t1.entry, ":")[1], split(t1.entry, ":")[2],
  split(t1.entry, ":")[3],split(t1.entry, ":")[4], split(t1.entry, ":")[5]
